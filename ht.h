...
{
    if(static_cast<double>(size_+amtDeleted_)/CAPACITIES[mIndex_] >= resizeAlpha_)
        resize();

    HASH_INDEX_T h = probe(p.first);
    if(h == npos)
        throw std::logic_error("No location is available to insert");
    if(table_[h] == nullptr){
...
{
    HashItem* hItem = internalFind(key);
    if(hItem != NULL){
        hItem->deleted = true;
        size_--;
        amtDeleted_++;
...
{
    std::vector<HashItem*> oTable = table_;
    mIndex_++;
    table_.clear();
    table_.resize(CAPACITIES[mIndex_], nullptr);
    size_ = 0;
    amtDeleted_ = 0;
    for(size_t i = 0; i < oTable.size(); i++){
        HashItem* hItem = oTable[i];
        if(hItem != nullptr && hItem->deleted == false){
            insert(hItem->item);
        }
        delete hItem;
    }
